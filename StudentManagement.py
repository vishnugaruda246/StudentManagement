...
#Adding student details
    ...
    def CheckAdmission_View(self):
        if self.getInfo_entry.get() == "":
            messagebox.showerror("Error!", "Please enter your Addmision number",parent=self.window)
        else:
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password, database=self.database)
                curs = connection.cursor()
                ...
                if row == None:
                    messagebox.showerror("Error!","Addmission number doesn't exists",parent=self.window)
                else:
    ...
    def ShowDetails(self, row):
        ...
        dob = Label(self.frame_1, text="DOB", font=(self.font_2, 15, "bold"), bg=self.color_1).place(x=300,y=240)
        ...
        email_data = Label(self.frame_1, text=row[9], font=(self.font_1, 10)).place(x=300, y=34
    ...
    def GetUpdateDetails(self, row):
        ...
        admission = Label(self.frame_1, text="Admission Number", font=(self.font_2, 15, "bold"), bg=self.color_1)
        admission_data = Label(self.frame_1, text=row[1], font=(self.font_1, 10)).place(x=40, y=60)
        ...
        self.dob = Label(self.frame_1, text="DOB", font=(self.font_2, 15, "bold"), bg=self.color_1).place(x=300,y=240)
        self.dob_entry = Entry(self.frame_1, bg=self.color_4, fg=self.color_3)
        ...
        if self.name_entry.get() == "" or self.surname_entry.get() == "" or self.course_entry.get() == "" or self.subject_entry.get() == "" or self.year_entry.get() == "" or self.age_entry.get() == "" or self.gender_entry.get() == "" or self.birth_entry.get() == "" or self.email_entry.get() == "":
            ...
            try:
                ...
                if row==None:
                    ...
                    curs.execute("update student set name=%s, course=%s, subject=%s, year=%s, age=%s, gender=%s, birth=%s, contact=%s, email=%s where admission_no=%s",
                                        (
                                            self.name_entry.get(),
                                            self.course_entry.get(),
                                            self.subject_entry.get(),
